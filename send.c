...
int mutt_fetch_recips(struct Envelope *out, struct Envelope *in, int flags)
{
  ...
  if ((flags & (SENDLISTREPLY | SENDGROUPREPLY)) && in->mail_followup_to)
  {
    ...
    if ((hmfupto = query_quadoption(HonorFollowupTo, prompt)) == MUTT_ABORT)
      return -1;
  }
                         ...
                         struct Context *ctx, struct Header *cur)
{
  ...
  if (flags & SENDREPLY)
  {
    if ((i = query_quadoption(Include, _("Include message in reply?"))) == MUTT_ABORT)
      return -1;
  ...
  {
    if ((i = query_quadoption(MimeForward, _("Forward as attachment?"))) == MUTT_YES)
    {
                    ...
                    struct Context *ctx, struct Header *cur)
{
  ...
  if (!flags && !msg && Recall != MUTT_NO && mutt_num_postponed(1))
  {
     ...
     */
    if ((i = query_quadoption(Recall, _("Recall postponed message?"))) == MUTT_ABORT)
      return rv;
